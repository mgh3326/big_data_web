...
def getPelicanaAddress(result):
    ...
    Pelicana_URL = 'http://www.weather.go.kr/weather/climate/past_cal.jsp?stn=108&yy=2018&mm=1&obs=1&x=28&y=11'
    rcv_data = get_request_url(Pelicana_URL)
    soupData = BeautifulSoup(rcv_data, 'html.parser', from_encoding='utf-8')
    print(soupData)
    store_table = soupData.find('table', attrs={'class': 'table_develop'})
